// backend/src/controllers/areasController.ts
...
export const createArea = async (req: Request, res: Response) => {
    try {
        ...
        // Si se subió un archivo físico, se genera la URL estática local
        let fotoFinal: string | null = null;
        if (req.file) {
            fotoFinal = `http://localhost:4000/uploads/areas/${req.file.filename}`;
        } else {
